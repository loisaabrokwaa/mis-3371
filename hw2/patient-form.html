...
function showErrorBanner(msg) {
    var existing = document.getElementById('submit-error-banner');
    if (!existing) {
        existing = document.createElement('div');
        existing.id = 'submit-error-banner';
        existing.style.cssText = 'background:#fdecea;border:2px solid #c0392b;color:#c0392b;padding:10px 16px;margin:10px 0;border-radius:6px;font-weight:700;font-size:0.88rem;';
        var form = document.getElementById('regForm');
        if (form) form.insertBefore(existing, form.firstChild);
    }
...
function hideErrorBanner() {
    var el = document.getElementById('submit-error-banner');
...
function submitForm(event) {
    ...
    if (!ok) {
        ...
        showErrorBanner('Some fields have errors. Please fix them before submitting.');
        reviewInput();
        var firstBad = document.querySelector('.field-error');
